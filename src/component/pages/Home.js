...
import { withTranslation, useTranslation, translate, Trans } from 'react-i18next';
...
class Home extends Component {
  ...
  changeBackground(x) {
    console.log("x: " + x)
    document.getElementById('background-container').classList.remove(this.myStyleClass)
  ...
  render() {
    ...
    */}
    ...
    const homeNavigation = i18n.t('home.navigation', { returnObjects: true }).map((item, i) => {
      if (0 == i) {
        return (
          <Scene duration={600} key={i}>
            <Col xs={12} md={9} className="h-100 my-auto ejemplo0" id={"trigger-" + i.toString()}
        ...
        return (
         ...
         Home
            ...
            <Col xs={12} md={7} className="h-100 mh-100 ejemplo" id={"trigger-" + i.toString()}
              onMouseOver={() => this.changeBackground(item.bk_img)}>
    ...
    return (
      ...
      <div id="container-home" className="h-100 mh-100">
        <div id="background-container" className="w-100 mw-100 h-100 mh-100 position-fixed scene_solutions"></div>
        <div id="seccions-container" className="d-flex flex-wrap flex-md-nowrap h-100 mh-100">
